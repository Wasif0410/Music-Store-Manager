...
public class Library
{
	...
	public void download(AudioContent content)
	{
		...
		// IF USER ENTERS INCORRECT INPUT VALUE
		if(content == null){
			throw new FaultyValueException("\nEnter Valid Input");
		}
		...
		if (content.getType().equals(Song.TYPENAME))
		{
			...
			// IF NOT DOWNLOADED
				...
				// PRINT OUT MESSAGE IF DOWNLOADED CORRECTLY
				System.out.println("SONG" + " " + content.getTitle()+ " Added To Library");
			}
		}
		
		// IF CONTENT IS AUDIOBOOK
		...
		if (content.getType().equals(Podcast.TYPENAME))
		{
			if (podcasts.contains(content))
			{
				...
				// THROW EXCEPTION
				throw new AlreadyDownloadedException(errorMsg);
			}
...
// CREATE CUSTOM EXCEPTION CLASS
class FaultyValueException extends RuntimeException
{
	public FaultyValueException(){}

	public FaultyValueException(String message){
		super(message);
	}
